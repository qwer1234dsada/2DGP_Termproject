...
class Monster:
    ...
    def __init__(self):
        if Monster.image == None:
            ...
            Monster.image2 = load_image('knight.png')
        self.x, self.y = random.randint(1000, 2500), 110
        self.movecount = 1000
        self.moveflag = 0
        self.swallow_suck = 0
        self.trun_amount = 0
    ...
    def draw(self):
        if self.type == 0:
            ...
            if self.swallow_finish_type == 0:
                if self.moveflag == 1:
                    if klrby.Camera_movex > 0 and klrby.Camera_movex < 800:
                        self.image2.clip_draw(0, 0, 30, 24, self.x - ( klrby.Camera_movex * 2 ), self.y, 50, 50)
                    else:
                        self.image2.clip_draw(0, 0, 30, 24, self.x, self.y, 50, 50)
                elif self.moveflag == 0:
                    if klrby.Camera_movex > 0 and klrby.Camera_movex < 800:
                        self.image2.clip_composite_draw(0, 0, 30, 24, -3.14, 'v', self.x - ( klrby.Camera_movex * 2 ), self.y, 50, 50)
                    else:
                        self.image2.clip_composite_draw(0, 0, 30, 24, -3.14, 'v', self.x, self.y, 50, 50)

                if self.swallow_suck == 1:
                    self.moveflag = 3
                    if klrby.global_klrby_x < self.x- ( klrby.Camera_movex*2):
                        if klrby.Camera_movex > 0 and klrby.Camera_movex < 800:
                            self.image2.clip_composite_draw(0, 0, 30, 24, self.trun_amount, '', self.x - ( klrby.Camera_movex * 2 ) - self.move_amount, self.y, 50, 50)
                        else:
                            self.image2.clip_composite_draw(0, 0, 30, 24,  self.trun_amount, '', self.x - self.move_amount, self.y, 50, 50)
                    elif klrby.global_klrby_x > self.x - ( klrby.Camera_movex*2):
                        if klrby.Camera_movex > 0 and klrby.Camera_movex < 800:
                            self.image2.clip_composite_draw(0, 0, 30, 24, self.trun_amount, '',self.x - (klrby.Camera_movex * 2) + self.move_amount, self.y, 50, 50)
                        else:
